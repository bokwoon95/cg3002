    ...
    for(;;){
        if (Serial1.available()) { // if there is an available byte..
            ...
            if (input == SYN) {      // RPI is trying to initiate handshake
                ...
                handshake_flag = 0;
                handle_handshake();
            } else if (input == DATA_I) { // RPI is requesting data
                if (handshake_flag) { 
                    if( xSemaphore_IMU != NULL ){
                        /* See if we can obtain the semaphore.  If the semaphore is not
                           available wait 10 ticks to see if it becomes free. */
...
void setup() {
    ...
    xTaskCreate(retrievePowerData, "retrievePowerData", STACK_DEPTH, (void *) NULL, 0, NULL);
    xTaskCreate(retrieveSensorData, "retrieveSensorData", STACK_DEPTH, (void *) NULL, 0, NULL);
